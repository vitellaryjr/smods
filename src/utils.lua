...
function SMODS.create_card(t)
    ...
    if t.seal then
        _card:set_seal(t.seal); _card.ability.delay_seal = false
    end
    if t.stickers then
...
G.FUNCS.SMODS_scoring_calculation_function = function(e)
    ...
    if G.GAME.current_scoring_calculation.update_ui then
        ...
        if operator and (first or (type(G.GAME.current_scoring_calculation.colour) == 'function' or type(G.GAME.current_scoring_calculation.text) == 'function')) then
            ...
            if card.edition then
                ...
                if edition.calc_scaling and type(edition.calc_scaling) == 'function' then
                    ...
                    if ret then
                        ...
                        if ret.override_scalar_value and not args.block_overrides.scalar then
                            scalar_value = ret.override_scalar_value.value; SMODS.calculate_effect(
                            ret.override_scalar_value, _card)
                        end
                        if ret.override_message and not args.block_overrides.message then scaling_message = SMODS
                            .merge_defaults(ret.override_message, scaling_message) end
                        if ret.post then
...
function SMODS.stake_is_unlocked(stake_key, deck_key)
    if not (SMODS.Stakes[stake_key] and G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key]) then return stake_key ==
        SMODS.stake_from_index(1) end
...
function CardArea:handle_card_limit()
    if SMODS.should_handle_limit(self) then
        if not G.TAROT_INTERRUPT then
            ...
            self.config.card_limits.total_slots = self.config.card_limits.extra_slots +
            (self.config.card_limits.base or 0) + (self.config.card_limits.mod or 0)
                        ...
                        G.E_MANAGER:add_event(Event({
                            ...
                            func = function()
                                if (self.config.card_limits.total_slots - self.config.card_count - (SMODS.cards_to_draw or 0)) > 0 and #G.deck.cards > (SMODS.cards_to_draw or 0) then
                                    G.FUNCS.draw_from_deck_to_hand(self.config.card_limits.total_slots -
                                    self.config.card_count - (SMODS.cards_to_draw or 0))
                                end
                                return true
                            end
                        }))
...
function SMODS.is_active_blind(key, ignore_disabled)
    return G.GAME and G.GAME.blind and G.GAME.facing_blind and
    (G.GAME.blind.name == key or G.GAME.blind.config.key == key) and (not G.GAME.blind.disabled or ignore_disabled)
end
    ...
    for _, hand in ipairs(args.hands) do
        ...
        if not instant then
            update_hand_text({ sound = 'button', volume = 0.7, pitch = 0.8, delay = 0.3 },
                { handname = localize(hand, 'poker_hands'), level = G.GAME.hands[hand].level })
            for name, p in pairs(SMODS.Scoring_Parameters) do
            ...
            if G.GAME.hands[hand][parameter] then
                ...
                if not instant then
                    ...
                    update_hand_text({ delay = 0 },
                        { [parameter] = G.GAME.hands[hand][parameter], StatusText = StatusText })
                end
            end
        end
        if args.level_up then
            context.old_level = G.GAME.hands[hand].level
            G.GAME.hands[hand].level = G.GAME.hands[hand].level +
            (type(args.level_up) == 'number' and args.level_up or 1)
...
SMODS.ease_types = {
    ...
    elastic = function(percent_done) return -math.pow(2, 10 * percent_done - 10) *
        math.sin((percent_done * 10 - 10.75) * 2 * math.pi / 3); end,
    inelastic = function(percent_done) return -math.pow(2, 10 * percent_done - 10) *
        math.sin((percent_done * 10 - 10.75) * 2 * math.pi / 3); end,
    outelastic = function(percent_done) return math.pow(2, -10 * percent_done) *
        math.sin((percent_done * 10 - 0.75) * (2 * math.pi) / 3) + 1 end,
    ...
    inback = function(percent_done, c1, c2, c3) return c3 * percent_done * percent_done * percent_done -
        c1 * percent_done * percent_done end,
    outback = function(percent_done, c1, c2, c3) return 1 + c3 * math.pow(percent_done - 1, 3) +
        c1 * math.pow(percent_done - 1, 2) end,
...
function SMODS.log_crash_info(info, defined)
    ...
    elseif source == "SMODS" then
        ...
        if modID == '_' then
            ...
            return string.format("\n\nError exists in %s in file '%s'%s at line %d", SMODS.Mods[modID].name,
...
function SMODS.create_canvas()
    local canvas = love.graphics.newCanvas(love.window.fromPixels(love.graphics.getWidth()) * G.CANV_SCALE,
        love.window.fromPixels(love.graphics.getHeight()) * G.CANV_SCALE, { type = '2d', readable = true })
